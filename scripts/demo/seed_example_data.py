...
def upsert_categoria(service: CategoriaService, session: Session, nombre: str, descripcion: str, orden: int, parent_id: int | None) -> int:
...
def upsert_ingrediente(service: IngredienteService, session: Session, nombre: str, descripcion: str, es_alergeno: bool, stock_actual: float, stock_minimo: float, costo_unitario: Decimal, unidad_medida: UnidadEnum) -> int:
...
def upsert_producto(service: ProductoService, session: Session, nombre: str, descripcion: str, precio_base: Decimal, categoria_id: int | None, disponible: bool, usa_stock_manual: bool, stock_manual: int | None, costo_compra_manual: Decimal | None, ingredientes: list[ProductoIngredienteSchema]) -> int:
...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        pizzas_id = upsert_categoria(categoria_service, session, "Pizzas", "Categoría principal de pizzas", 1, None)
        bebidas_id = upsert_categoria(categoria_service, session, "Bebidas", "Bebidas frías y sin alcohol", 2, None)
        especiales_id = upsert_categoria(categoria_service, session, "Pizzas especiales", "Pizzas con variantes especiales", 3, pizzas_id)
        tradicionales_id = upsert_categoria(categoria_service, session, "Pizzas tradicionales", "Recetas clásicas", 4, pizzas_id)

        queso_id = upsert_ingrediente(
            ingrediente_service,
            session,
            "Queso Mozzarella",
        ...
        upsert_producto(
            ...
            None,
            [
                ProductoIngredienteSchema(ingrediente_id=masa_id, cantidad=300, unidad=UnidadEnum.GRAMOS, es_removible=False, es_opcional=False),
                ProductoIngredienteSchema(ingrediente_id=salsa_id, cantidad=120, unidad=UnidadEnum.GRAMOS, es_removible=True, es_opcional=False),
                ProductoIngredienteSchema(ingrediente_id=queso_id, cantidad=250, unidad=UnidadEnum.GRAMOS, es_removible=True, es_opcional=False),
            ],
        )
        ...
        upsert_producto(
            ...
            [
                ProductoIngredienteSchema(ingrediente_id=masa_id, cantidad=300, unidad=UnidadEnum.GRAMOS, es_removible=False, es_opcional=False),
                ProductoIngredienteSchema(ingrediente_id=salsa_id, cantidad=100, unidad=UnidadEnum.GRAMOS, es_removible=True, es_opcional=False),
                ProductoIngredienteSchema(ingrediente_id=queso_id, cantidad=220, unidad=UnidadEnum.GRAMOS, es_removible=True, es_opcional=False),
                ProductoIngredienteSchema(ingrediente_id=jamon_id, cantidad=120, unidad=UnidadEnum.GRAMOS, es_removible=True, es_opcional=True),
            ],
        )
