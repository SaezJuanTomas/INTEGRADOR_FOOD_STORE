...
export function PerfilPage(): JSX.Element {
  ...
  const savePerfilMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      window.alert("Perfil actualizado. Si no ves cambios en el encabezado, recarga la página.");
    },
  });
  ...
  const saveDireccionMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      window.alert("Dirección guardada");
    },
  ...
  if (perfilQuery.isLoading) {
    return <p className="text-slate-700">Cargando perfil...</p>;
  }
  ...
  if (direccionesQuery.isLoading) {
    return <p className="text-slate-700">Cargando direcciones...</p>;
  }
